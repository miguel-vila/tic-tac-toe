...
(defn send-text [text]
  (let [message (.stringify js/JSON text)]
    (js/console.log (str "About to send: " message))
    (.send ws message)))

(defn onmessage [msg]
  (let [x (.parse js/JSON (.-data msg))]
    (js/console.log "Got message from server:" (pr-str x))))

(set! (.-onmessage ws) onmessage)

(comment
(go
  (let [{:keys [ws-channel]} (<! ws)
    ...
    (render-state [_ state]
                  (let [view-opts {:opts opts}]
                    (dom/div #js {:className "game"}
                             ...
                             (dom/div nil
                                      (dom/input #js {:className "centered"
                                                      :value (:text state)
                                                      :onChange #(handle-change % owner state)})
                                      (dom/button #js {:onClick (fn [_] (send-text (:text state)))} "Send!"))
